...
<html>
  ...
  <head>
    ...
    $(function(){
    	$("#deptForm").submit(function(){
    		...
    		else if (!/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/.test($.trim(cardId.val()))){
  ...
  <body>
    <div class="x-body">
        <form class="layui-form" method="POST" id="deptForm"  action="${pageContext.request.contextPath}/employee/edit">
          ...
          <div class="layui-form-item" >
            ...
            <div class="layui-form-item" >
              <label for="sexId" class="layui-form-label">
             ...
             <div class="layui-form-item" >
              <label for="educationId" class="layui-form-label">
                  <span class="x-red">*</span>学历
              </label>
              <div class="layui-input-inline">
               <select id="educationId" name="educationId" class="valid">
                  			<c:forEach items="${requestScope.education_list}" var="edu">
						   		<c:choose>
			    					<c:when test="${employee.educationId== edu.id }">
			    						<option value="${edu.id }" selected="selected">${edu.name }</option>
			    					</c:when>
			    					<c:otherwise>
